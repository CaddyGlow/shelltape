...
fn copy_hook_file(shelltape_dir: &PathBuf, shell: Shell) -> Result<()> {
    ...
    fs::write(&hook_file_path, hook_content).with_context(|| {
        format!(
            "Failed to write hook file to: {}",
            hook_file_path.display()
        )
    })?;

    println!("  ✓ Copied hook file to {}", hook_file_path.display());
...
fn add_to_rc_file(shell: Shell) -> Result<()> {
    ...
    if !rc_path.exists() {
        fs::write(&rc_path, "").with_context(|| {
            format!("Failed to create RC file: {}", rc_path.display())
        })?;
    }

    // Read existing content to check if already installed
    let content =
        fs::read_to_string(&rc_path).with_context(|| format!("Failed to read: {}", rc_path.display()))?;
    ...
    if content.contains(&hook_line) {
        println!("  ℹ Shelltape hooks already present in {}", rc_path.display());
        return Ok(());
    }
    ...
    println!("  ✓ Added hooks to {}", rc_path.display());
